 ...
 // const { isLoggedIn } = useAuth();
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const categoriesResponse = await axios.get(`http://10.108.0.4:3002/get-categories/${userId}?month=${selectedMonth}`, {
          headers: {
  ...
  const handleAddExpense = async () => {
    try {
      ...
      await axios.post('http://10.108.0.4:3002/add-expense', {
        userId: userId,
        month: selectedMonth,
