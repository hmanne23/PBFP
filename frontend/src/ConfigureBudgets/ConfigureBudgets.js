...
function ConfigureBudgets() {
  ...
  const handleAddBudget = async () => {
    if (category && budget && selectedMonth) {
      ...
      try {
        const response = await axios.get(`http://167.172.242.243:3002/check-existing-budget/${userId}/${selectedMonth}/${category}`,{
          headers:{
  ...
  const handleSaveBudgets = async () => {
    try {
      ...
      console.log(months);
      await axios.post(
        "http://167.172.242.243:3002/configure-budgets",
        {
          userId,
